...
for line in lines:
    ...
    for pos, char in enumerate(line):
        if char.isdigit():
            positions_and_values.append((pos, char))
    for key, value in number_dict.items():
        ...
        for pos in positions:
            positions_and_values.append((pos, key))

    sorted_positions_and_values = dict(sorted(positions_and_values))
    first_key, first_value = next(iter(sorted_positions_and_values.items()))
    last_key, last_value = next(reversed(sorted_positions_and_values.items()))
